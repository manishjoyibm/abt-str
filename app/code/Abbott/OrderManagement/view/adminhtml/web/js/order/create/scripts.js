...
data = {};
            ...
            this.flagstore = data.flagstore;
            this.similac_store_id = data.similac_store_id;
            ...
            jQuery.async('#order-items', (function () {
                ...
                let searchButtonId = 'add_products',
        ...
        setStoreId : function (id, configFlag = 0, store = "") {

            let infoSpan = jQuery("#data-column-info");
            let errorFlag = 0;
            let shareConfig = 0;
            let customerStoreId = 0;
            if (infoSpan) {
                shareConfig = infoSpan.attr("data-column2");
                customerStoreId = infoSpan.attr("data-column1");
                if (configFlag == 1) {
                    shareConfig = 1;
                }
            }

            if (configFlag != 0 && this.customerId == false) {
                alert({
                    content: "Customer creation not allowed for this store"
                });
                return;
            }

            if (shareConfig == 1 && this.customerStore.trim() != "" && store.trim() != "" && this.customerStore.trim() != store.trim()) {
                errorFlag = 1;
            } else if (shareConfig == 1 && customerStoreId != 0 && customerStoreId != id) {
                errorFlag = 1;
            }
            if (errorFlag == 1) {
                alert({
                    content: "Selected customer does not belong to this store."
                });
                return;
            }
        ...
        loadShippingRates: function () {
            let addressContainer = this.shippingAsBilling ?
        ...
        loadPaymentMethods: function () {
            let data = this.serializeData(this.billingAddressContainer).toObject();
        ...
        setPaymentMethod : function (method) {
            if (this.paymentMethod && $('payment_form_'+this.paymentMethod)) {
                let form = 'payment_form_'+this.paymentMethod;
...
elem.disabled = true;
        ...
        addProduct : function (id) {
            this.loadArea(['items', 'shipping_method', 'totals', 'billing_method'], true, {add_product:id, qty: 5, reset_shipping: true});
        },

        addProductButton : function (element, id) {
            let qty = jQuery(element).closest("tr").find('input[name="qty"]');
            this.loadArea(['items', 'shipping_method', 'totals', 'billing_method'], true, {add_product:id, qty: qty.val(), reset_shipping: true});
            //this.hideArea('search');
            this.gridProducts = $H({});
            let scrollPos = jQuery("#order-items").offset().top;
        ...
        productGridFilterKeyPress: function (grid, event) {
            let returnKey = parseInt(Event.KEY_RETURN || 13, 10);
...
Element.show(this.productGridShowButton);
        ...
        selectCustomer : function (grid, event) {
            ...
            if (element.title) {
                let trObj = jQuery("tr[title='" + element.title + "']");
                if (trObj) {
                    this.customerStore = trObj.find("[data-column='store_name']").html();
                }
...
show = true; }
        ...
        validateVat: function (parameters) {
            ...
            let currentCustomerGroupId = $(parameters.groupIdHtmlId)
        ...
        processCustomerGroupChange: function (groupIdHtmlId, message, customerGroupMessage, errorMessage, groupId, action) {
            ...
            try {
                ...
                let currentCustomerGroupTitle =
                    $$('#' + groupIdHtmlId + ' > option[value=' + currentCustomerGroupId + ']')[0].text;
                let customerGroupOption = $$('#' + groupIdHtmlId + ' > option[value=' + groupId + ']')[0];
