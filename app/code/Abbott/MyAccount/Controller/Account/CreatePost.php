...
class CreatePost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    const ADDRESSEDIT = 'customer/address/edit';
    ...
    public function createCsrfValidationException(
        ...
        $url = $this->urlModel->getUrl(self::CREATE, [self::SECURE => true]);
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($this->helper->getConfigDomainRestriction(Data::ENABLE_DOMAIN_RESTRICTION) &&
                $this->helper->verifyDomain($customer->getEmail())) {
                throw new InputException(__(
                    $this->helper->getConfigDomainRestriction(
                        Data::ERROR_MESSAGE_DISPOSABLE_EMAIL_DOMAN
                    )
                ));
            }
            ...
            $customer = $this->accountManagement
                ...
                $cartId = $this->helper->setAllCookies($customer, $password);
                $this->sgpRestriction->setRestrictionDetails();
                $guestCartKey = $this->getCookieManager()->getCookie('abt_cartKey');
            ...
            if ($confirmationStatus === AccountManagementInterface::ACCOUNT_CONFIRMATION_REQUIRED) {
                ...
                $url = $this->urlModel->getUrl('*/*/index', [self::SECURE => true]);
                $resultRedirect->setUrl($this->_redirect->success($url));
            } else {
                $this->session->setCustomerDataAsLoggedIn($customer);
                $redirectUrl = $this->getCookieManager()->getCookie('redirectUrl');
                if ($redirectUrl) {
                    $cookieDomain = $this->helper->getCookieRedirect();
                    ...
                    $this->setCookie(
                        'abt_msg',
                        '{"type":"success","message":"' . $this->getSuccessMessage()
                        . '"}',
        ...
        $defaultUrl = $this->urlModel->getUrl(self::CREATE, [self::SECURE => true]);
    ...
    protected function getSuccessMessage()
    {
        if ($this->addressHelper->isVatValidationEnabled()) {
            if ($this->addressHelper->getTaxCalculationAddressType() == Address::TYPE_SHIPPING) {
                ...
                $message = __(
                    'If you are a registered VAT customer, please <a href="%1">click here</a> to
enter your shipping address for proper VAT calculation.',
                    $this->urlModel->getUrl(self::ADDRESSEDIT)
                );
                ...
                $message = __(
                    'If you are a registered VAT customer, please <a href="%1">click here</a> to
enter your billing address for proper VAT calculation.',
                    $this->urlModel->getUrl(self::ADDRESSEDIT)
                );
                // @codingStandardsIgnoreEnd
            }
