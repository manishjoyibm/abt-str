...
namespace Abbott\AdditionalAttributes\DataProvider\Product\LayeredNavigation\Builder;

use Abbott\AdditionalAttributes\DataProvider\Product\LayeredNavigation\AttributeOptionProvider;
        ...
        foreach ($this->getAttributeBuckets($aggregation) as $bucket) {
            ...
            if ($this->storeManager->getStore()->getCode() == AccountHelper::NEW_SIM_STORE_CODE) {
                ...
                $metrics = $value->getMetrics();
                    $result[$bucketName]['options'][$metrics['value']] = $this->layerFormatter->buildItem(
                ...
                foreach ($attribute['options'] as $optionid => $option) {
                    if (($option !== null) && (!array_key_exists($optionid, $result[$bucketName]['options']))) {
                        $result[$bucketName]['options'][] = $this->layerFormatter->buildItem(
                            $attribute['options'][$optionid] ?? $optionid,
                            $optionid,
                            0
                    );
                        $optioncount++;
                ...
                $result[$bucketName]['count'] = $optioncount;
            } else {
                foreach ($bucket->getValues() as $value) {
                    $metrics = $value->getMetrics();
                    $result[$bucketName]['options'][] = $this->layerFormatter->buildItem(
