...
namespace Abbott\Sarp2\Engine\Profile\Action\Type\ChangePlan;
...
/**
 * Class Applier
 * @package Abbott\Sarp2\Engine\Profile\Action\Type\ChangePlan
 */
class Applier extends \Aheadworks\Sarp2\Engine\Profile\Action\Type\ChangePlan\Applier
{
    /**
     * @var ResultFactory
     */
    ...
    const CRONTAB_CONST_GLUCERNA = "crontab";
    ...
    public function validate(ProfileInterface $profile, ActionInterface $action)
    {
        ...
        if ($isValid) {
            foreach ($profile->getItems() as &$item) {
                ...
                $newOption = $this->getOptionByPlan($item->getProductId(), $newPlanId,$profile->getStoreId());
                if (!$newOption) {
                    $isValid = false;
     ...
     */
    private function getOptionByPlan($productId, $newPlanId, $storeId=null)
    {
        $subscriptionOptions = $this->optionsRepository->getList($productId,$storeId);
    ...
    {
        if($this->state->getAreaCode() == self::CRONTAB_CONST_GLUCERNA &&  $profile->getStoreId() == AccountHelper::GLU_STORE_ID)
        {             
            $connection = $this->resource->getConnection();
            $planDefTable = $connection->getTableName('aw_sarp2_plan_definition');
            $plan_def_select = $connection->select()->from($planDefTable)->where('definition_id = ?', $profile->getPlanDefinitionId());                
            $data = $connection->fetchRow($plan_def_select);
            $days = $data['billing_frequency'];            
            $profileDefinition = $profile->getProfileDefinition();
            $regularTotalCount = $profileDefinition->getIsMembershipModelEnabled() && $profileDefinition->getTotalBillingCycles()
            ? $profileDefinition->getTotalBillingCycles() + 1
            : $profileDefinition->getTotalBillingCycles();
            $schedule->setPeriod($profileDefinition->getBillingPeriod())->setFrequency($days)->setTrialTotalCount(0)->setTrialCount(0)->setRegularTotalCount($regularTotalCount)
            ->setIsMembershipModel(
            $profileDefinition->getIsMembershipModelEnabled()  ? $profileDefinition->getIsMembershipModelEnabled() : false);
            $schedule
            ->setIsInitialPaid(0)
            ->setTrialCount(0)
            ->setRegularCount(0);
