<?php
namespace Abbott\Sarp2\Engine\Notification\Scheduler\Type;
...
class UpcomingBilling implements SchedulerInterface
{
    ...
    public function schedule($sourcePayment)
    {
        ...
        $scheduleOffset = $profileDefinition->getUpcomingBillingEmailOffset()
            ? : $this->config->getUpcomingBillingEmailOffset($storeId);

        $offset = $this->config->getEarliestNextPaymentDate();
        if ($scheduleOffset && $sourcePayment->getType() != PaymentInterface::TYPE_LAST_PERIOD_HOLDER) {
            $estimated = (new \DateTime($sourcePayment->getScheduledAt()))
                ->modify('-' . $offset . ' day');
            $estimatedTm = $this->coreDate->gmtTimestamp($estimated);
            $estimatedSchedule = (new \DateTime($sourcePayment->getScheduledAt()))
                ->modify('-' . $scheduleOffset . ' day');
            ...
            if ($estimatedTm >= $todayTm) {
                ...
                $notification->setType(NotificationInterface::TYPE_UPCOMING_BILLING)
                    ...
                    ->setScheduledAt($estimatedSchedule)
                    ->setStoreId($storeId)
