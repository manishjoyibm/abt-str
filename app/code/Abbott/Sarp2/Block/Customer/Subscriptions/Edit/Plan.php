...
class Plan extends Template
{
    ...
    public function getConfigData()
    {
        ...
        $appPrice = $this->getPlanPrice($this->getProfile());
        foreach ($newDetails as &$newDetail) {
          foreach ($newDetail as &$detail) {
            if($detail['type'] == Processor::REGULAR_PAYMENT) {
              $billingCycles = $detail['composite_value']['billing_cycles'];
              $price = $this->priceCurrency->format($appPrice, false);
              $detail['composite_value']['value'] = $appPrice;
              $detail['value'] = $billingCycles > 0
                  ? __('%1 x %2', $billingCycles, $price)
                  : $price;
            }
          }
    ...
    /**
     * @param  \Aheadworks\Sarp2\Api\Data\ProfileInterface $profileItems
     * @return float
     */
    private function getPlanPrice($profileData) {
      $totalArray = [];
      $profileItems = $profileData->getItems();
      foreach ($profileItems as $item) {
        $regularPrice = $this->priceCalculate->getAutoRegularPrice($item->getProductId(), $profileData->getPlanId());
        $regularProductPrice = $this->priceCalculate->getSubscriptionCustomerGroupPrice($item->getProductId(), $regularPrice, $profileData->getCustomerId());
