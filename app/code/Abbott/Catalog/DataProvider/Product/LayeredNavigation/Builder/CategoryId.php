...
namespace Abbott\Catalog\DataProvider\Product\LayeredNavigation\Builder;
...
use
 Magento\CatalogGraphQl\DataProvider\Product\LayeredNavigation\Builder\Aggregations\Category\IncludeDirectChildrenOnly;
 ...
 */
class CategoryId implements LayerBuilderInterface
{
    /**
     * @var string
     */
    private const CATEGORY_BUCKET_ID = 'category_id_bucket';

    /**
     * @var array
     */
    private static $bucketMap = [
        self::CATEGORY_BUCKET_ID => [
     ...
     */
    private IncludeDirectChildrenOnly $includeDirectChildrenOnly;
    ...
    public function build(AggregationInterface $aggregation, ?int $storeId): array
    {
        ...
        $bucket = $aggregation->getBucket(self::CATEGORY_BUCKET_ID);
        ...
        $result = $this->layerFormatter->buildLayer(
            self::$bucketMap[self::CATEGORY_BUCKET_ID]['label'],
            \count($categoryIds),
            self::$bucketMap[self::CATEGORY_BUCKET_ID]['request_name']
        );
        ...
        foreach ($bucket->getValues() as $value) {
            ...
            if (array_key_exists($categoryId, $categoryLabels)) {
