define([
    ...
    $(document).on('change', '.agreement_braintree_paypal_new_similac,.custom_check', function(e) {
      if($('.dynamic_text').length && $('.payment-method._active').attr('id') == 'payment-method-braintree-paypal'){ 
        if($('.custom_check').prop("checked") == true && $('.agreement_braintree_paypal_new_similac').prop("checked") == true){
        ...
        if($('.agreement_braintree_paypal_new_similac').prop("checked") == true){
          $("#braintree_paypal_placeholder").css({"opacity": "1", "pointer-events": "all"});   
          $('.custom_checkout_error').remove();       
        } else {
          $("#braintree_paypal_placeholder").css({"opacity": "0.3", "pointer-events": "none"});        
