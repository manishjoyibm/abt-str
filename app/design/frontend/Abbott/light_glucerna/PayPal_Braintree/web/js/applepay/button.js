                    ...
                    self.collectDeviceData(clientInstance, context, function () {
                        applePay.create({
                            ...
                            el.addEventListener('click', function (e) {
                                ...
                                var label = jQuery("input[name='amastyPaymentMethodAttributes[glucerna_terms]']").attr('id');
                                var code = label.split("-")[0];
                                if (!jQuery("input[name='amastyPaymentMethodAttributes[glucerna_terms]']").is(":checked")) {
                                    if (jQuery(".validate-applepay").length == 0) {
                                        var html = '<div for="agreement[1]" generated="true" class="mage-error validate-applepay" id="agreement[1]-error">This is a required field.</div>';
                                        if (jQuery("#error-" + code).length == 1) {
                                            return false;
                                        } else {
                                            jQuery("label[for=" + label + "]").append(html);
                                        }
                                    }
                                    return false;
                                } else {
