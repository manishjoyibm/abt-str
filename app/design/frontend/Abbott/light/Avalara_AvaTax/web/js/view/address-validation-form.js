    ...
    function (
        ...
        return {
            ...
            fillValidateForm: function (form) {
                this.reset(form);
                if($("#street_1").val() != undefined) {
                    var street = $("#street_1").val().toLowerCase();
                    if (street.indexOf("po box") != -1) {
                        var errString = $.mage.__('We do not ship to PO BOX');
                        addressModel.error(errString);                        
                    }
                }                
            ...
            buildValidAddress: function (originalAddress, validAddress) {
                ...
                if (typeof originalAddress.region_code !== 'undefined') {
					...
					if(originalAddress.region == "") {
						originalAddress.region = $("#region_id option:selected" ).text();
					}
